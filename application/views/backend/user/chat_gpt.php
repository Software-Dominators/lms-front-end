...
<div class="tab-content" id="pills-tabContent">
	...
	<div class="tab-pane fade" id="pills-gpt-service" role="tabpanel" aria-labelledby="pills-gpt-service-tab">
		<form class="aiAjaxFormSubmission ai_from" action="<?php echo site_url('user/chat_gpt') ?>" method="post">
...
<script type="text/javascript">
			...
			complete: function(xhr) {
				...
				if (/^[\],:{}\s]*$/.test(response.replace(/\\["\\\/bfnrtu]/g, '@').replace(/"[^"\\\n\r]*"|true|false|null|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?/g, ']').replace(/(?:^|:|,)(?:\s*\[)+/g, '')) && $('#ai_service_selector').val() == 'Course thumbnail') {
					...
					for (let i = 0; i < parsedJson.length; i++) {
						var exi = i + 1;
						var img = '<div class="w-50 p-2 position-relative"><a class="position-absolute btn btn-success px-1 py-0 m-1" href="<?php echo site_url('user/ai_img_download?index='); ?>' + exi + '" target="_blank"><i class="fas fa-download"></i></a><img class="radius-5px" width="100%" src="' + parsedJson[i].url + '"></div>';
	...
	function gpt_assistant() {
		...
		$.ajax({
			type: 'post',
			url: '<?php echo site_url('user/gpt_assistant') ?>',
			data: {
